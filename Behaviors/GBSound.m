/*
 * GBSound.m
 * GershwinBehaviors - system sound playback
 *
 ...
 *
 * TODO: Upstream to GNUstep - libs-gui's libao-backed NSSound should honor
 * -setVolume: (or the back should expose per-sound gain) so callers do not
 * need to pre-scale PCM samples themselves.
 */
...
static BOOL GBParseWav(const uint8_t *b, NSUInteger len, GBWavInfo *out)
{
...
static int32_t GBClamp(double v, double lo, double hi)
{
...
static NSData *GBScaleWav(NSData *wav, float gain)
{
    const uint8_t *src = [wav bytes];
    GBWavInfo info = {0};

    if (!GBParseWav(src, [wav length], &info))
        return nil;
            ...
            switch (info.bitsPerSample) {
                case 8: {
                    ...
                    for (NSUInteger i = 0; i < n; i++) {
                        int v = GBClamp((d[i] - 128) * gain, -128, 127);
                        d[i] = (uint8_t)(v + 128);
                    }
                    break;
                }
                case 16: {
                    for (NSUInteger i = 0; i + 1 < n; i += 2) {
                        ...
                        s = (int16_t)GBClamp(s * gain, -32768, 32767);
                        d[i] = (uint8_t)(s & 0xff);
                ...
                case 24: {
                    for (NSUInteger i = 0; i + 2 < n; i += 3) {
                        ...
                        s = GBClamp(s * gain, -8388608, 8388607);
                        d[i] = (uint8_t)(s & 0xff);
                ...
                case 32: {
                    for (NSUInteger i = 0; i + 3 < n; i += 4) {
                        int32_t s = (int32_t)((uint32_t)d[i] | ((uint32_t)d[i + 1] << 8) |
                                              ((uint32_t)d[i + 2] << 16) |
                                              ((uint32_t)d[i + 3] << 24));
                        s = GBClamp((double)s * gain, -2147483648.0, 2147483647.0);
                        d[i] = (uint8_t)(s & 0xff);
                        d[i + 1] = (uint8_t)((s >> 8) & 0xff);
                        d[i + 2] = (uint8_t)((s >> 16) & 0xff);
...
static float GBAlertVolumeGain(void)
{
...
BOOL GBPlaySystemSound(NSString *soundName)
{
    NSArray *soundPaths = @[
        ...
        for (NSString *ext in extensions) {
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:soundPath]) {
                ...
                if (gain < 0.99) {
                    ...
                    NSData *quiet = wav ? GBScaleWav(wav, gain) : nil;
                    if (quiet) {
