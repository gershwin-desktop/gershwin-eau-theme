// NSMenuView+GB.m
// NSMenuView behavior: submenu positioning, keyboard navigation and
// scrollable overflowing menus.
//
...
// matching Mac OS X 10.5 (Leopard) behavior.  The scroll arrows are a look,
// so the theme draws them (Eau: NSMenuView+Eau.m) from GBMenuScrollManager.
...
static NSRect s_gb_rectOfItemAtIndex(id self, SEL _cmd, NSInteger index)
{
  ...
  if (![menuView isHorizontal])
    {
      GBMenuScrollManager *mgr = [GBMenuScrollManager scrollManagerForMenuView: menuView];
...
static void s_gb_sizeToFit(id self, SEL _cmd)
{
  ...
  if (mgr && [mgr isScrolling])
  ...
  else
    {
      ...
      // frame methods in NSMenu+GB.m saw no menu view and silently
      // skipped overflow setup.  Run overflow detection *now* that
      // the view is definitely in the window.
      [GBMenuScrollManager setupOverflowForMenuView: menuView];
    }
}
...
static NSInteger s_gb_nextSelectableItem(NSMenuView *menuView, NSInteger from, NSInteger dir)
{
...
static void s_gb_warpToItem(NSMenuView *menuView, NSInteger index)
{
  if (index < 0) return;
...
static void s_gb_switchToAdjacentMenu(NSMenuView *menuView, NSInteger dir)
{
  ...
  NSInteger next = s_gb_nextSelectableItem(root, cur >= 0 ? cur : (dir > 0 ? -1 : [[root menu] numberOfItems]), dir);
  if (next >= 0)
    {
      ...
      s_gb_warpToItem(root, next);
    }
}

static void s_gb_keyDown(id self, SEL _cmd, NSEvent *event)
{
...
// NSLog(@"GB+Menu: keyDown chars=%@ mods=0x%lx hasMod=%d self=%@",
...
// NSLog(@"GB+Menu: keyDown c=0x%04x cur=%ld horiz=%d isAttached=%d",
  ...
  switch (c)
    {
      ...
      case NSUpArrowFunctionKey:
        ...
        newIdx = s_gb_nextSelectableItem(menuView, cur >= 0 ? cur : 0, -1);
        // NSLog(@"GB+Menu: Up cur=%ld newIdx=%ld", (long)cur, (long)newIdx);
        if (newIdx >= 0)
          {
            ...
            if (parentIdx >= 0)
              {
                ...
                s_gb_warpToItem(parent, parentIdx);
              }
        ...
        if (isHoriz)
          {
            if (cur < 0)
              {
                newIdx = s_gb_nextSelectableItem(menuView, -1, 1);
                // NSLog(@"GB+Menu: Down first newIdx=%ld", (long)newIdx);
            ...
            // NSLog(@"GB+Menu: Down open submenu for idx=%ld item=%@ submenu=%d enabled=%d",
            //       (long)cur, [item title], [item submenu] != nil, [item isEnabled]);
            if (item && [item submenu] && [item isEnabled])
              {
                [menuView attachSubmenuForItemAtIndex: cur];
                s_gb_warpToItem(menuView, cur);
              }
          }
        else
          {
            newIdx = s_gb_nextSelectableItem(menuView, cur >= 0 ? cur : -1, 1);
            // NSLog(@"GB+Menu: Down cur=%ld newIdx=%ld", (long)cur, (long)newIdx);
            if (newIdx >= 0)
              [menuView setHighlightedItemIndex: newIdx];
          }
        return;
      ...
      case NSRightArrowFunctionKey:
        if (isHoriz)
          {
            s_gb_switchToAdjacentMenu(menuView, 1);
          }
        else
          {
            if (cur >= 0)
...
static void s_gb_mouseDown(id self, SEL _cmd, NSEvent *event)
{
  s_orig_mouseDown(self, _cmd, event);
}

// trackWithEvent: not swizzled here - NSMenu+GB.m owns that swizzle
...
static void s_gb_setHighlightedItemIndex(id self, SEL _cmd, NSInteger index)
{
...
static void GBInstallNSMenuViewSwizzles(void);

@implementation NSMenuView (GBBehaviors)

+ (void)load
{
  GBInstallNSMenuViewSwizzles();
}

/* TODO: Upstream to GNUstep - -[NSMenuView locationForSubmenu:] should keep
 * dropdowns and submenus on screen (flip left, shift up) like this does. */

- (NSPoint)gb_locationForSubmenu:(NSMenu *)aSubmenu
{
  NSDebugLog(@"NSMenuView+GB: gb_locationForSubmenu: called for submenu %@", aSubmenu);
  ...
  if ([menuView isHorizontal])
    {
      ...
      if (yPos < 0)
        {
          NSDebugLog(@"NSMenuView+GB: Horizontal menu extends past bottom border (yPos=%.1f), shifting up", yPos);
          yPos = 0;
        }
      ...
      NSDebugLog(@"NSMenuView+GB: Horizontal pos for '%@': itemRect=%@ screenOrigin=%@ subH=%.1f → (%.1f, %.1f)",
  ...
  if (yPos < 0)
    {
      NSDebugLog(@"NSMenuView+GB: Vertical submenu extends past bottom border (yPos=%.1f), shifting up", yPos);
      yPos = 0;
    }
  ...
  NSDebugLog(@"NSMenuView+GB: Vertical pos for '%@': parentFrame=%@ itemScreen=%@ itemH=%.1f subH=%.1f → (%.1f, %.1f)",
...
// Runs from +load when the bundle is loaded
static void GBInstallNSMenuViewSwizzles(void)
{
  Class menuViewClass = objc_getClass("NSMenuView");
  if (!menuViewClass)
    {
      NSDebugLog(@"NSMenuView+GB: ERROR - NSMenuView class not found");
      return;
    }
  ...
  // --- Swizzle locationForSubmenu: ---
  {
    ...
    SEL swizzledSelector = @selector(gb_locationForSubmenu:);
        ...
        if (originalIMP != swizzledIMP)
          {
            ...
            NSDebugLog(@"NSMenuView+GB: Swizzled locationForSubmenu:");
          }
      }
    else
      {
        NSDebugLog(@"NSMenuView+GB: Could not swizzle locationForSubmenu: (orig=%p swiz=%p)",
    ...
    if (m)
      {
        ...
        method_setImplementation(m, (IMP)s_gb_rectOfItemAtIndex);
        NSDebugLog(@"NSMenuView+GB: Swizzled rectOfItemAtIndex: for scroll support");
      }
  }
    ...
    if (m)
      {
        ...
        method_setImplementation(m, (IMP)s_gb_sizeToFit);
        NSDebugLog(@"NSMenuView+GB: Swizzled sizeToFit for overflow height preservation");
      }
  }
    ...
    if (m)
      {
        ...
        method_setImplementation(m, (IMP)s_gb_setHighlightedItemIndex);
        NSDebugLog(@"NSMenuView+GB: Swizzled setHighlightedItemIndex: for scroll-into-view");
      }
  }
    ...
    if (nsresp)
      {
        ...
        class_addMethod(menuViewClass, sel, (IMP)s_gb_keyDown, enc);
        // NSLog(@"GB+Menu: Installed keyDown: on NSMenuView (orig=%p new=%p enc=%s)",
        //       s_orig_keyDown, s_gb_keyDown, enc);
      }
  }
    ...
    if (m)
      {
        ...
        method_setImplementation(m, (IMP)s_gb_mouseDown);
        NSDebugLog(@"NSMenuView+GB: Swizzled mouseDown: for edge autoscroll");
      }
  }
