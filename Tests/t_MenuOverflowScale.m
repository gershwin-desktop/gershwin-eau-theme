/* t_MenuOverflowScale.m - a menu taller than the screen scrolls inside a
 ...
 *
 * Runs with the plain GNUstep theme, so the installed Eau bundle does not
 * load its own copy of the scroll manager into this process.  Needs DISPLAY.
 *
...
int main(int argc, char **argv)
{
  ...
  START_SET("overflow at scale 1.1")
    ...
    PASS([EauMenuScrollManager setupOverflowForMenuView: view],
    ...
    EauMenuScrollManager *mgr = [EauMenuScrollManager scrollManagerForMenuView: view];
    ...
    [EauMenuScrollManager setupOverflowForMenuView: view];
  ...
  START_SET("a submenu opened lower down")
    ...
    PASS([EauMenuScrollManager setupOverflowForMenuView: subView], "it scrolls too");
    sf = [subWindow frame];
