...
@implementation NSOutlineView (Eau)

+ (void) load
{
  // Exchange the initWithFrame: method to enable alternating row colors by default
  Method originalInit = class_getInstanceMethod([NSOutlineView class], @selector(initWithFrame:));
  Method eauInit = class_getInstanceMethod([NSOutlineView class], @selector(eau_initWithFrame:));
  ...
  if (self)
    {
      ...
      // Set row height to match HIG full-size control height
      [self setRowHeight: METRICS_TEXT_INPUT_FIELD_HEIGHT];
